...
export default defineConfig({
	...
	run: {
		tasks: {
			...
			'inertia-app:build': { command: 'vp build --config apps/inertia/app/vite.config.js', cache: false },
			'inertia-app:dev': { command: 'vp dev --config apps/inertia/app/vite.config.js', cache: false },
			'inertia:e2e': { command: 'vp run inertia-app:build && vp exec node apps/inertia/tests/e2e/runner.playwright.mjs --target alloy', cache: false },
			'inertia:e2e:agent-browser': { command: 'vp run inertia-app:build && vp exec node apps/inertia/tests/e2e/runner.mjs --target alloy', cache: false },
			'inertia:e2e:bedrock': { command: 'vp exec node apps/inertia/tests/e2e/runner.mjs --target bedrock --build-app', cache: false },
