// Package command is the CLI dispatch table. A Set is a named group of
// Commands with its own usage text and error exit code, so nested groups (a
// parent Command whose Run is a child Set's Dispatch) need no branching code.
//
// treex deliberately hand-rolls dispatch rather than taking a CLI framework
// dependency: the command surface is small and fixed, and the module ships as
// a distributed binary where every transitive dependency is a supply-chain
// surface on a tool that deletes files.
package command
