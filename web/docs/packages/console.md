...
```bash
GOWORK=./packages/foundation/go.work go test -count=1 ./packages/foundation/console/...
```
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/foundation/console` cover the supported creation paths, default values, and parity behavior.
