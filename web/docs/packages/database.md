...
```bash
GOWORK=./pkg/hub/go.work go test -count=1 ./pkg/hub/database/...
```
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `pkg/hub/database` cover the supported creation paths, default values, and parity behavior.
