...
```bash
go get github.com/oullin/alloy/pkg/hub/collection@latest
```

When working inside this monorepo, use the repository workspace:

```bash
GOWORK=./pkg/hub/go.work go test -count=1 ./pkg/hub/collection/...
```
...
import (
    _ "github.com/oullin/alloy/pkg/hub/collection"
)
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `pkg/hub/collection` cover the supported creation paths, default values, and parity behavior.
...
- `pkg/hub/collection/collection/inventory_parity_additional_test.go`
