...
```bash
GOWORK=./pkg/hub/go.work go test -count=1 ./pkg/hub/httppreview/...
```
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `pkg/hub/httppreview` cover the supported creation paths, default values, and parity behavior.
...
- `pkg/hub/httppreview/inventory_parity_test.go`
