...
```bash
GOWORK=./packages/foundation/go.work go test -count=1 ./packages/foundation/search/...
```
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/foundation/search` cover the supported creation paths, default values, and parity behavior.
...
- `packages/foundation/search/builder_inventory_test.go`
- `packages/foundation/search/engines/collection_inventory_test.go`
- `packages/foundation/search/engines/database_inventory_test.go`
