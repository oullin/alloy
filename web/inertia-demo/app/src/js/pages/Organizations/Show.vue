...
const props = withDefaults(defineProps<{ organization: Organization; contacts?: CursorPaginated<Contact> }>(), {
	contacts: () => ({ data: [], next_cursor: null }),
});

const breadcrumbs = [{ title: 'CRM' }, { title: 'Organizations', href: organizationRoutes.index().url }, { title: props.organization.name }];
			...
			<Card>
				<CardHeader class="flex flex-row items-center justify-between gap-4">
					...
					<Button size="sm" as-child>
						<Link :href="`${contactRoutes.create().url}?organization_id=${organization.id}`">Add member</Link>
					</Button>
				</CardHeader>
				<CardContent>
					<div v-if="allContacts.length === 0" class="text-muted-foreground text-sm">No contacts in this organization.</div>
					<div v-else class="space-y-2">
						<Link v-for="contact in allContacts" :key="contact.id" :href="contactRoutes.show(contact.id).url" class="flex items-center gap-3 rounded-lg bg-muted/30 p-3 hover:bg-muted/50">
							<div class="bg-primary/10 text-primary flex size-8 shrink-0 items-center justify-center rounded-full text-xs font-medium">
					...
					<div v-if="nextCursor" class="flex justify-center pt-4">
						<Button variant="outline" size="sm" @click="loadMoreContacts">Load more members...</Button>
					</div>
				</CardContent>
			</Card>
		</div>
