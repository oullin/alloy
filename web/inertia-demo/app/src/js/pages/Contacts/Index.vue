	...
	<AppLayout title="Contacts" :breadcrumbs="breadcrumbs">
		<div class="flex h-full flex-1 flex-col gap-4 p-4">
			...
			<div class="flex flex-col gap-3 md:flex-row md:items-center">
				...
				<Button variant="outline" :class="{ 'bg-accent': favoriteOnly }" @click="favoriteOnly = !favoriteOnly"> Favorites </Button>
			</div>
			...
			<div class="space-y-2">
				...
				>
					<div class="bg-primary/10 text-primary flex size-10 shrink-0 items-center justify-center rounded-full text-sm font-medium">
						{{ contact.first_name?.[0] ?? '' }}{{ contact.last_name?.[0] ?? '' }}
					</div>
					<div class="min-w-0 flex-1">
						<div class="flex items-center gap-2">
							<span class="font-medium">{{ contact.first_name }} {{ contact.last_name }}</span>
							<span v-if="contact.is_favorite" class="text-xs font-medium text-red-500">Favorite</span>
						</div>
						<div class="text-muted-foreground truncate text-sm">{{ contact.email }}</div>
					</div>
					<Badge v-if="contact.organization" variant="secondary">{{ contact.organization.name }}</Badge>
				</Link>

				<div v-if="allContacts.length === 0" class="text-muted-foreground py-12 text-center">No contacts found.</div>
			</div>
