...
const breadcrumbs = [{ title: 'CRM' }, { title: 'Contacts', href: contactRoutes.index().url }, { title: `${props.contact.first_name} ${props.contact.last_name}` }];
				...
				<CardHeader class="flex flex-row items-start justify-between gap-4">
					<div class="space-y-2">
						<div class="flex items-center gap-2">
							<CardTitle class="text-2xl tracking-tight"> {{ contact.first_name }} {{ contact.last_name }} </CardTitle>
			...
			<Card>
				...
				<CardContent class="space-y-3">
					<form class="space-y-3" @submit.prevent="submitNote">
						...
						<Textarea id="note-body" v-model="noteForm.body" name="body" placeholder="Write a note for the activity feed..." />
			...
			<Card>
				...
				<CardContent>
					<Deferred data="notes">
						...
						<div v-if="!notes || notes.length === 0" class="text-muted-foreground text-sm">No notes yet.</div>
						<div v-else class="space-y-3">
							<div v-for="note in notes" :key="note.id" class="rounded-lg bg-muted/30 p-4">
								<div class="mb-2 flex items-center justify-between gap-3">
									...
									<time class="text-muted-foreground text-xs">{{ new Date(note.created_at).toLocaleString() }}</time>
								</div>
