...
withDefaults(defineProps<{ currentTime?: string; randomNumber?: number; contactCount?: number }>(), {
	currentTime: '',
	randomNumber: 0,
	contactCount: 0,
});
...
<template>
	<AppLayout title="Polling" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Polling" description="Automatically refresh page data at regular intervals using usePoll. This page polls every 5 seconds." />
			...
			<div class="grid gap-6 lg:grid-cols-3">
				<FeatureCard title="Current Time" description="Server time, refreshed every 5 seconds.">
					...
					<p class="text-muted-foreground mt-2 text-xs">This value comes from the server and updates automatically.</p>
				</FeatureCard>
			...
			<FeatureCard title="How It Works" description="usePoll under the hood.">
				<div class="space-y-3 text-sm">
					<p>
						<code class="bg-muted rounded px-1.5 py-0.5 text-xs">usePoll(5000)</code> sets up an interval that calls
						<code class="bg-muted rounded px-1.5 py-0.5 text-xs">router.reload()</code>
						every 5 seconds. The poll automatically pauses when the tab is not visible and resumes when you return.
					</p>
					<p>
						You can control polling with the returned
						<code class="bg-muted rounded px-1.5 py-0.5 text-xs">stop()</code> and <code class="bg-muted rounded px-1.5 py-0.5 text-xs">start()</code> functions.
					</p>
				</div>
			</FeatureCard>
		</div>
