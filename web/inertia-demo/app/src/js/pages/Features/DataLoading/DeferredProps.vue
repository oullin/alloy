...
withDefaults(
	defineProps<{
		quickStat?: number;
		slowStats?: Record<string, unknown>;
		heavyData?: Array<{ name?: string; status?: string }>;
	}>(),
	{
		quickStat: 0,
		slowStats: undefined,
...
<template>
	<AppLayout title="Deferred Props" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Deferred Props" description="Deferred props allow you to load slow data after the initial page render, showing skeleton fallbacks until the data arrives." />
			...
			<div class="grid gap-6 lg:grid-cols-3">
				...
				<FeatureCard title="Slow Stats" description="These stats are deferred and load after the initial render.">
				...
				<FeatureCard title="Heavy Data" description="A large dataset loaded in the background.">
					<Deferred data="heavyData">
						...
						<div class="space-y-2">
							<div v-for="(item, index) in heavyData" :key="index" class="flex items-center justify-between rounded-md border p-2 text-sm">
								<span>{{ item.name ?? item }}</span>
								<Badge v-if="item.status" variant="secondary">{{ item.status }}</Badge>
							</div>
							<p v-if="!heavyData?.length" class="text-muted-foreground text-sm">No data available.</p>
						</div>
					</Deferred>
				</FeatureCard>
			</div>
