...
<template>
	<AppLayout title="Optional Props" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Optional Props" description="Optional props are not included in the initial page load. They are only sent when explicitly requested via partial reloads." />
			...
			<div class="grid gap-6 lg:grid-cols-3">
				<FeatureCard title="Regular Data" description="Always included in every response.">
					<div class="space-y-2">
						...
						<pre class="bg-muted mt-2 overflow-auto rounded p-3 text-xs">{{ JSON.stringify(regularData, null, 2) }}</pre>
					</div>
				</FeatureCard>
				...
				<FeatureCard title="Optional Data" description="Only sent when explicitly requested.">
					<div class="space-y-2">
						...
						<template v-if="optionalData">
							<pre class="bg-muted overflow-auto rounded p-3 text-xs">{{ JSON.stringify(optionalData, null, 2) }}</pre>
						</template>
						<p v-else class="text-muted-foreground text-sm">Click "Load Optional Data" to fetch this prop. It is skipped in the initial page load to reduce payload size.</p>
					</div>
				</FeatureCard>
				...
				<FeatureCard title="Deferred Data" description="Loaded automatically after initial render.">
					<Deferred data="deferredData">
						...
						<div class="space-y-2">
							...
							<pre class="bg-muted overflow-auto rounded p-3 text-xs">{{ JSON.stringify(deferredData, null, 2) }}</pre>
						</div>
					</Deferred>
				</FeatureCard>
			</div>
