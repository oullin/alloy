...
const breadcrumbs = [{ title: 'Features' }, { title: 'Prefetching' }, { title: 'Cache Management' }];
...
<template>
	<AppLayout title="Cache Management" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Cache Management" description="Understand the cache lifecycle for prefetched pages and how to manage cache invalidation." />
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Cache Lifecycle" description="How prefetch cache entries are managed.">
					<div class="space-y-3 text-sm">
						<div class="flex items-start gap-3 rounded-md border p-3">
							...
							<p>The cached response is within the fresh window. It will be served directly without any network request.</p>
						</div>
						<div class="flex items-start gap-3 rounded-md border p-3">
							<Badge class="shrink-0" variant="secondary">Stale</Badge>
							<p>The response is past its fresh window but still within the stale window. It is served immediately while a background revalidation runs.</p>
						</div>
						<div class="flex items-start gap-3 rounded-md border p-3">
							<Badge class="shrink-0" variant="outline">Expired</Badge>
							<p>The cache entry has exceeded both windows and is discarded. A full request is made on the next navigation.</p>
						</div>
					</div>
				</FeatureCard>

				<FeatureCard title="Cache Invalidation" description="Ways to clear or invalidate cached data.">
					<div class="space-y-4">
						<div class="space-y-3 text-sm">
							<p>Cached responses are automatically invalidated when a non-GET request (POST, PUT, DELETE) is made, since those requests typically change server state.</p>
				...
				<FeatureCard title="cacheFor Options" description="Different ways to configure cache duration.">
					<div class="space-y-3">
						<div class="rounded-md border p-3">
							<p class="text-sm font-medium">Number (milliseconds)</p>
							<code class="text-muted-foreground text-xs">:cache-for="30000"</code>
							<p class="text-muted-foreground mt-1 text-xs">Cache for 30 seconds (fresh only, no stale window).</p>
						</div>
						<div class="rounded-md border p-3">
							<p class="text-sm font-medium">Array [fresh, stale]</p>
							<code class="text-muted-foreground text-xs">:cache-for="[5000, 60000]"</code>
							<p class="text-muted-foreground mt-1 text-xs">Fresh for 5s, then stale-while-revalidate up to 60s.</p>
						</div>
						<div class="rounded-md border p-3">
				...
				<FeatureCard title="Best Practices" description="Tips for effective cache management.">
					<ul class="space-y-2 text-sm">
						...
						<li class="flex items-start gap-2">
							...
							Consider using
							<code class="bg-muted rounded px-1 py-0.5 text-xs">prefetch="mount"</code> for links the user is very likely to click.
						</li>
					</ul>
				</FeatureCard>
			</div>
