			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Trigger Visits" description="Each visit has its own set of callbacks.">
					<div class="space-y-4">
						...
						<p class="text-muted-foreground text-sm">
							Unlike global events, visit callbacks are scoped to a single request. They are passed as options to
							<code class="bg-muted rounded px-1.5 py-0.5 text-xs">router.post()</code>, <code class="bg-muted rounded px-1.5 py-0.5 text-xs">router.visit()</code>, etc.
						</p>
					</div>
				</FeatureCard>
				...
				<FeatureCard title="Callback Log" :description="`${callbackLog.length} callback(s) fired`">
					<div class="max-h-80 space-y-1.5 overflow-auto">
						<div v-for="entry in callbackLog" :key="entry.id" class="flex items-center gap-2 rounded-md border p-2 text-xs">
							<Badge
								variant="outline"
								class="shrink-0"
								:class="{
						...
						<p v-if="callbackLog.length === 0" class="text-muted-foreground py-4 text-center text-sm">No callbacks fired yet. Click a button to trigger a visit.</p>
					</div>
				</FeatureCard>
			</div>

			<FeatureCard title="Callback Reference" description="Available per-visit callbacks and when they fire.">
