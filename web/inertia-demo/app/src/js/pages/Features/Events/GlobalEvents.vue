...
function logEvent(name: string, detail: string | object) {
	eventLog.value.unshift({
		id: ++idCounter,
		name,
		detail: detail !== null && typeof detail === 'object' ? JSON.stringify(detail) : String(detail ?? ''),
...
<template>
	<AppLayout title="Global Events" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Global Events" description="Listen to Inertia router events globally using router.on(). Events fire during the lifecycle of every Inertia visit." />
			...
			<div class="grid gap-6 lg:grid-cols-2">
				...
				<FeatureCard title="Event Log" :description="`${eventLog.length} event(s) captured`">
					<div class="max-h-80 space-y-1.5 overflow-auto">
						<div v-for="entry in eventLog" :key="entry.id" class="flex items-start gap-2 rounded-md border p-2 text-xs">
							<Badge
								variant="outline"
								class="shrink-0"
								:class="{
						...
						<p v-if="eventLog.length === 0" class="text-muted-foreground py-4 text-center text-sm">No events captured yet. Click "Send POST Request" to trigger some.</p>
					</div>
				</FeatureCard>
			</div>
