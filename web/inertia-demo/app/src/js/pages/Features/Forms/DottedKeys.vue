	...
	<AppLayout title="Dotted Keys" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Nested Form Fields" description="Uses dotted notation to represent nested data structures.">
						...
						<div>
							...
							<div class="grid gap-3">
								<div class="grid gap-2">
									...
									<Input id="email0" v-model="form.contact.emails[0]" type="email" placeholder="primary@example.com" />
									<InputError :message="form.errors['contact.emails[0]']" />
								</div>

								<div class="grid gap-2">
									<Label for="email1">contact.emails[1]</Label>
									<Input id="email1" v-model="form.contact.emails[1]" type="email" placeholder="secondary@example.com" />
				...
				<div class="grid gap-6">
					<FeatureCard title="Form Data" description="The current form data using dotted key notation.">
						<div class="text-sm">
							<pre class="bg-muted overflow-auto rounded-md p-3 text-xs">{{ JSON.stringify(form.data(), null, 2) }}</pre>
						</div>
					</FeatureCard>

					<FeatureCard title="How Dotted Keys Work" description="Understanding the flat-to-nested mapping.">
						<div class="grid gap-2 text-sm">
							<div class="rounded-md border p-3">
								<p class="mb-1 font-medium">Flat Keys</p>
								<p class="text-muted-foreground">
									Form fields use flat dotted notation like
									<code class="bg-muted rounded px-1">address.street</code> and <code class="bg-muted rounded px-1">contact.emails[0]</code>.
								</p>
							</div>
							<div class="rounded-md border p-3">
								...
								<p class="text-muted-foreground">
									...
									<code class="bg-muted rounded px-1">ParseForm</code> flattens it back to dotted keys so handlers access values via
