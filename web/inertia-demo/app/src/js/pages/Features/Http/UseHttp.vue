	...
	<AppLayout title="useHttp" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="API Form" description="Submit to a JSON endpoint without page navigation.">
				...
				<FeatureCard title="Response" description="JSON response from the API endpoint.">
					...
					<div v-else-if="error" class="space-y-2">
						...
						<pre class="overflow-auto rounded border border-red-200 bg-red-50 p-3 text-xs text-red-700">{{ typeof error === 'string' ? error : JSON.stringify(error, null, 2) }}</pre>
					</div>

					<div v-else-if="response" class="space-y-2">
						<Badge>Success</Badge>
						<pre class="bg-muted overflow-auto rounded p-3 text-xs">{{ JSON.stringify(response, null, 2) }}</pre>
					</div>

					<p v-else class="text-muted-foreground text-sm">Submit the form to see the JSON response here. The page will not navigate or reload.</p>
				</FeatureCard>
			</div>
			...
			<FeatureCard title="When to Use" description="JSON APIs vs Inertia visits.">
				<div class="space-y-3 text-sm">
					<p>
						...
						<code class="bg-muted rounded px-1.5 py-0.5 text-xs">router.visit()</code> or <code class="bg-muted rounded px-1.5 py-0.5 text-xs">useForm</code>, which trigger full page
						responses. However, some use cases benefit from plain JSON endpoints:
					</p>
					<ul class="space-y-2">
