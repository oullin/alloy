...
<template>
	<AppLayout title="HTTP Exceptions" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="HTTP Exceptions" description="Handle server-side HTTP errors (403, 404, 500, etc.) gracefully in your Inertia application." />

			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Trigger Errors" description="Click to trigger different HTTP error responses.">
					<div class="space-y-4">
						...
						<p class="text-muted-foreground text-sm">These buttons send POST requests that trigger the server to respond with the corresponding HTTP status code.</p>
					</div>
				</FeatureCard>

				<FeatureCard title="Last Exception" description="The most recent error caught by the listener.">
					...
					<p v-else class="text-muted-foreground text-sm">No exceptions caught yet. Trigger an error using the buttons.</p>
				</FeatureCard>
			</div>

			<FeatureCard title="Error Handling Strategies" description="Different ways to handle HTTP errors.">
				<div class="space-y-4">
					<div class="rounded-md border p-3">
						<p class="text-sm font-medium">Error Pages</p>
						<p class="text-muted-foreground mt-1 text-sm">By default, Inertia renders error pages for non-Inertia responses. You can customize these by creating error page components.</p>
					</div>
					<div class="rounded-md border p-3">
