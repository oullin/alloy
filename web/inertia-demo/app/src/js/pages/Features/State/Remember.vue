...
import { ref } from 'vue';
import { useRemember, router, Link } from '@inertiajs/vue3';
	...
	<AppLayout title="Remember" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="useRemember Form" description="This form state persists across navigation.">
					<div class="space-y-4">
						<div class="grid gap-3">
							...
							<div class="grid gap-1.5">
								...
								<Input id="email" v-model="formState.email" type="email" placeholder="you@example.com" />
							</div>
							<div class="grid gap-1.5">
						...
						<div class="flex items-center gap-2">
							<Link v-if="flashDataRoute" :href="flashDataRoute" class="inline-flex items-center rounded-md border px-3 py-2 text-sm font-medium hover:bg-accent"> Navigate Away </Link>
							<span class="text-muted-foreground text-xs">then come back to see data preserved</span>
						</div>
					</div>
				</FeatureCard>

				<FeatureCard title="Manual Remember/Restore" description="Using router.remember() and router.restore() directly.">
					<div class="space-y-4">
						...
						<p class="text-muted-foreground text-sm">
							...
							<code class="bg-muted rounded px-1.5 py-0.5 text-xs">router.remember()</code> and restored with <code class="bg-muted rounded px-1.5 py-0.5 text-xs">router.restore()</code>.
