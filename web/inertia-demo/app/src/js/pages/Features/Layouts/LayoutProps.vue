...
<template>
	<AppLayout :title="layoutTitle" :breadcrumbs="layoutBreadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Layout Props" description="Pass data from page components to their layout. The layout receives props like title and breadcrumbs from each page." />

			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Props Passed to Layout" description="Data this page sends to AppLayout.">
					<div class="space-y-2">
						...
						<div class="rounded-md border p-3">
							...
							<div class="flex flex-wrap gap-1">
								<Badge v-for="(crumb, index) in currentLayoutProps.breadcrumbs" :key="index" variant="secondary" class="text-xs">
									{{ crumb.title }}
								</Badge>
							</div>
						</div>
...
&lt;/template&gt;</pre>
				...
				<FeatureCard title="Persistent Layout Props" description="How props work with defineOptions layout.">
					<div class="space-y-3 text-sm">
						<p>
							When using persistent layouts via
							<code class="bg-muted rounded px-1.5 py-0.5 text-xs">defineOptions</code>, the layout receives page props through the Inertia page object rather than direct Vue prop binding.
						</p>
						<pre class="bg-muted overflow-auto rounded p-3 text-xs leading-relaxed">
