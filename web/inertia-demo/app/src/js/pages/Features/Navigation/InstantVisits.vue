	...
	<AppLayout title="Instant Visits" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Instant Visit Link" description="This link prefetches the request on mount so it's ready before you click.">
					<div class="flex flex-wrap gap-3">
						<Link :href="targetRoute.url" prefetch="mount" class="inline-flex h-9 items-center rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground hover:bg-primary/90">
							Visit target page (instant)
						</Link>
					</div>
					<p class="text-muted-foreground mt-2 text-xs">The request starts on mount/hover, so by the time you click the response is often already cached.</p>
				</FeatureCard>

				<FeatureCard title="Normal Visit Link" description="A standard link for comparison. The request starts after the click event.">
					<div class="flex flex-wrap gap-3">
						<Link
					...
					<p class="text-muted-foreground mt-2 text-xs">This is a regular Inertia link with no prefetching.</p>
				</FeatureCard>

				<FeatureCard title="How It Works" description="Instant visits leverage Inertia's prefetching to preload page data.">
					<div class="space-y-2 text-sm">
						...
						<p>When the user clicks, the cached response is used immediately if available, making the navigation appear instantaneous.</p>
					</div>
				</FeatureCard>

				<FeatureCard title="Usage" description="Add the prefetch attribute to Link components.">
					<pre class="overflow-auto rounded-md bg-muted p-3 text-xs leading-relaxed"><code>&lt;Link href="/target" prefetch="mount"&gt;
  Instant visit
&lt;/Link&gt;
