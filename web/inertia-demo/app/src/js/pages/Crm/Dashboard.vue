	...
	<AppLayout title="Dashboard" :breadcrumbs="breadcrumbs">
		<div class="flex h-full flex-1 flex-col gap-6 p-4">
			...
			<Card>
				...
				<CardContent>
					<div v-if="recentActivity.length === 0" class="text-muted-foreground py-4 text-sm">No recent activity.</div>
					<div v-else class="space-y-2">
						<div v-for="note in recentActivity" :key="note.id" class="flex items-start gap-4 rounded-lg bg-muted/30 px-4 py-3">
							<div class="flex-1 space-y-1">
								<div class="flex items-center gap-2">
									...
									<Link v-if="note.contact" :href="contactRoutes.show(note.contact?.id).url" class="text-sm font-medium text-primary hover:underline">
										{{ note.contact?.name }}
									</Link>
									<span v-else class="text-sm font-medium text-muted-foreground">Unknown contact</span>
								</div>
