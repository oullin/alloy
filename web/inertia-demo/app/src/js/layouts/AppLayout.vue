...
import { IconDashboard, IconUsers, IconCode, IconFileText, IconDots, IconLogout } from '@tabler/icons-vue';
import { Avatar, AvatarFallback } from '@/js/components/ui/avatar';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from '@/js/components/ui/dropdown-menu';
import LoadingBar from '@/js/components/ui/loading-bar/LoadingBar.vue';
import { Separator } from '@/js/components/ui/separator';
import { appRoutes, contactRoutes, organizationRoutes, featureRoute, useDemoRoute } from '@/js/lib/routes';
...
const title = computed(() => props.title || ((page.props as Record<string, unknown>).title as string) || '');

const breadcrumbs = computed(() => (props.breadcrumbs.length ? props.breadcrumbs : (((page.props as Record<string, unknown>).breadcrumbs as Breadcrumb[]) ?? [])));
...
function scrollToActiveNavItem() {
	nextTick(() => {
		...
		requestAnimationFrame(() => {
			document.querySelector('[data-sidebar="content"] [data-active="true"]')?.scrollIntoView({ block: 'nearest' });
		});
	});
}
...
const featureGroups = computed(() => [
	{
		...
		items: [
			...
			{ title: 'HTTPPreview', href: featureRoute('features.forms.httppreview') },
			...
			{ title: 'RouteGen', href: featureRoute('features.forms.routegen') },
		],
	},
	{
		label: 'Navigation',
...
<template>
	...
	>
					...
					<SidebarGroupContent>
						...
						<SidebarMenu>
							<SidebarMenuItem v-for="item in group.items" :key="item.title">
								<SidebarMenuButton as-child :is-active="currentPath === item.href || currentPath.startsWith(item.href + '/')" :tooltip="item.title">
					...
					<SidebarGroupContent>
						...
						<SidebarMenu>
							<SidebarMenuItem v-for="item in fGroup.items" :key="item.title">
								<SidebarMenuButton v-if="item.href" as-child :is-active="currentPath === item.href || currentPath.startsWith(item.href + '/')" :tooltip="item.title">
									<Link :href="item.href" view-transition>
						...
						<DropdownMenu>
							...
							<DropdownMenuContent side="top" align="start" class="w-(--reka-popper-anchor-width)">
								<DropdownMenuItem as-child>
									<Link :href="appRoutes.logout().url" method="post" as="button" class="w-full cursor-pointer">
		...
		<SidebarInset>
			<header class="bg-background/90 sticky top-0 z-10 flex h-(--header-height) shrink-0 items-center gap-2 border-b">
			...
			<div
				...
				:class="{
					...
					'border-blue-200 bg-blue-50 text-blue-800': page.props.flash.kind === 'info' || !page.props.flash.kind,
				}"
			>
