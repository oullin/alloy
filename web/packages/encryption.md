...
```bash
go get alloy.dev/go/encryption@latest
```

When working inside this monorepo, use the repository workspace:

```bash
GOWORK=./web/storage/.cache/go.work go test -count=1 ./packages/encryption/...
```
...
Use package tests as executable examples when the exact constructor requires collaborators. The tests under `packages/encryption` cover the supported creation paths, default values, and parity behavior.
