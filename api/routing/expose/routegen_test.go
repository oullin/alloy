...
import (
	...
	"github.com/oullin/alloy/routing/expose"
)
...
func generateTo(t *testing.T, routes []*expose.RouteInfo, opts expose.Options) string {
	t.Helper()

	dir := t.TempDir()
	opts.Path = dir

	if err := expose.Generate(routes, opts); err != nil {
...
func postControllerRoutes() []*expose.RouteInfo {
	base := "App\\Http\\Controllers\\PostController"

	return []*expose.RouteInfo{
...
func TestAnonymousMiddlewareClosureRoutesDoNotBlockGeneration(t *testing.T) {
	...
	dir := generateTo(t, routes, expose.Options{})
	content := readFile(t, dir, "actions/App/Http/Controllers/PostController.ts")
	...
	routes := []*expose.RouteInfo{
		{
			...
			Params:     []expose.Param{{Name: "parameter", Optional: true}},
		},
		{
	...
	routes := []*expose.RouteInfo{
		{
			...
			Params:     []expose.Param{{Name: "userProfile", Key: "uuid"}},
		},
	}
	...
	routes := []*expose.RouteInfo{
		{
			...
			Params:     []expose.Param{{Name: "user-profile", Key: "uuid"}},
		},
	}
	...
	routes := []*expose.RouteInfo{
		{
			...
			Params:     []expose.Param{{Name: "key", Key: "uuid"}},
		},
	}
	...
	routes := []*expose.RouteInfo{
		{
			...
			Domain:     "example.test",
			Scheme:     "//",
			Params:     []expose.Param{{Name: "param"}},
		},
		{
...
func TestNamedRoutesGeneration(t *testing.T) {
	...
	dir := generateTo(t, routes, expose.Options{SkipActions: true})
...
func TestGenerateFromRouter(t *testing.T) {
	...
	if err := expose.GenerateFromRouter(router, expose.Options{Path: dir}, expose.AdapterOptions{}); err != nil {
...
func TestWithFormOption(t *testing.T) {
	...
	dir := generateTo(t, routes, expose.Options{WithForm: true})
	content := readFile(t, dir, "actions/App/Http/Controllers/PostController.ts")
...
func TestExposeRuntimeUtility(t *testing.T) {
	...
	dir := generateTo(t, nil, expose.Options{SkipActions: true, SkipRoutes: true})
...
func TestExposeRuntimeDirectoryCompatibility(t *testing.T) {
	...
	dir := generateTo(t, nil, expose.Options{
		SkipActions:      true,
		SkipRoutes:       true,
...
func TestSkipOptions(t *testing.T) {
	...
	t.Run("skip_routes", func(t *testing.T) {
		t.Parallel()
		dir := generateTo(t, routes, expose.Options{SkipRoutes: true})
