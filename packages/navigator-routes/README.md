...
The TypeScript package lives in `packages/navigator-routes` and exposes
`@alloy/navigator-routes`. It is a tiny, dependency-free resolver that turns
...
```ts
import { createRouteResolver } from '@alloy/navigator-routes';
...
```ts
import { fillPattern, resolveRoute, resolveRouteUrl } from '@alloy/navigator-routes';
...
with `pnpm --filter @alloy/navigator-routes test`.
