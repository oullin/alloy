import type { CalendarFormatKey, FormatOptions, HumanDiffOptions, TimeZoneNameStyle } from '#types';
...
export const calendarFormatDefaults = (): Record<CalendarFormatKey, string> => ({
...
export const timeZoneName = (date: Date, timeZone: string, style: TimeZoneNameStyle, locale: string): string => {
	const parts = getFormatter(timeZone, style, locale).formatToParts(date);

	return readPart(parts, 'timeZoneName') || timeZone;
};

export const formatOffset = (offsetMinutes: number, separator: ':' | ''): string => {
...
export const bestRelativeUnit = (milliseconds: number): NonNullable<HumanDiffOptions['unit']> => {
...
export const unitDivisor = (unit: NonNullable<HumanDiffOptions['unit']>): number => {
	switch (unit) {
		case 'second':
...
export class TempoFormatter {
	format(
		...
		return pattern.replace(/\[[^\]]*]|YYYY|MMMM|dddd|MMM|ddd|SSS|Do|YY|ZZ|MM|DD|HH|hh|mm|ss|Z|X|x|Y|M|D|H|h|m|s|A|a|d/g, (token) =>
			token.startsWith('[') && token.endsWith(']') ? token.slice(1, -1) : (replacements[token] ?? token),
		);
	}
}
