...
The TypeScript package lives in `packages/workflow` and exposes
`@alloy/workflow`. It implements workflow-net and state-machine engines in
...
```ts
import { DefinitionBuilder, SingleStateStore, StateMachine } from '@alloy/workflow';
...
```ts
import { Dispatcher, EventNames } from '@alloy/workflow';
...
| --- | --- | --- |
| `@alloy/workflow` | `Machine`, `StateMachine`, `Definition`, `DefinitionBuilder`, `Marking`, `Transition` | core engine, definitions, markings |
| `@alloy/workflow` | `Dispatcher`, `EventNames`, `TransitionError`, `TransitionNotFoundError` | lifecycle events and coded failures |
| `@alloy/workflow/stores` | `SingleStateStore`, `MultiStateStore`, `MarkingStore` | persisting markings on subjects |
| `@alloy/workflow/events` | `GuardEvent`, `TransitionEvent`, `EnteredEvent`, ... | typed lifecycle event classes |
| `@alloy/workflow/audit` | `AuditTrail`, `AuditEntry` | recording applied transitions |
| `@alloy/workflow/registry` | `WorkflowRegistry`, `WorkflowRegistryEntry` | resolving machines per subject |
| `@alloy/workflow/config` | `WorkflowConfigLoader` | building definitions from config sources |
| `@alloy/workflow/validator` | `WorkflowValidator` | definition validation rules |
| `@alloy/workflow/multisteps` | `MultiStepWorkflow`, job/engine helpers | dependency-graph job execution |

Acceptance tests live in `packages/workflow/tests` and run with
`pnpm --filter @alloy/workflow test`.
