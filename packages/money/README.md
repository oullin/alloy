...
The TypeScript package lives in `packages/money` and exposes `@alloy/money`.
...
```ts
import { ExchangeRates, MoneyConverter, MoneyManager } from '@alloy/money';
...
```ts
import { MoneyParser } from '@alloy/money';
...
| --- | --- | --- |
| `@alloy/money` | everything below | root export |
| `@alloy/money` | `MoneyValue`, `MoneyManager`, `MoneyAggregator`, `MoneyConverter`, `MoneyJson` | value type, arithmetic, aggregation, conversion, JSON codecs |
| `@alloy/money/calculator` | `MoneyCalculator`, `MAX_INT64`, `MIN_INT64` | overflow-safe bigint arithmetic |
| `@alloy/money/currency` | `CurrencyManager`, `CurrencyDefinition`, `CurrencyMap`, `DefaultCurrencyProvider`, `ISOCodePattern` | currency registry and definitions |
| `@alloy/money/errors` | `MoneyError`, `MoneyErrors`, `ERR_*` constants | coded error values |
| `@alloy/money/exchange` | `ExchangeRates`, `ExchangeConverter` | exchange-rate tables and guarded access |
| `@alloy/money/format` | `MoneyFormatter` | display formatting and major-unit conversion |
| `@alloy/money/parser` | `MoneyParser` | parsing money strings into amount + currency |

Acceptance tests live in `packages/money/tests` and run with
`pnpm --filter @alloy/money test`.
