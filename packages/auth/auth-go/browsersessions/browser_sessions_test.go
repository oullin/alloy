...
func TestServiceListsSessionsAndMarksCurrent(t *testing.T) {
	repo := browsersessions.NewMemoryRepository(
		browsersessions.Session{ID: "current", UserID: "1"},
		browsersessions.Session{ID: "other", UserID: "1"},
		browsersessions.Session{ID: "other-user", UserID: "2"},
	)
...
func TestSQLRepositoryUsesSafeTableAndUserScopedDeletes(t *testing.T) {
	...
	repo := browsersessions.NewSQLRepository(db, "sessions; DROP TABLE users")
...
func (db *sessionSQLDB) Query(_ context.Context, query string, _ ...any) (browsersessions.SQLRows, error) {
	db.lastQuery = query
