...
BACKEND_PATH="${ROOT_PATH}/packages/backend"
GO_WORK_PATH="${ROOT_PATH}/go.work"
GO_WORK_ENV="off"
...
# Every Go module under packages/backend owns a go.mod.
while IFS= read -r -d '' gomod; do
	...
	(
		...
		if [[ "${module_path}" != "alloy.dev/backend" && "${module_path}" != alloy.dev/backend/* ]]; then
...
done < <(find "${BACKEND_PATH}" -name go.mod -print0 | sort -z)
