	...
	<AppLayout title="Manual Visits" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="HTTP Methods" description="Each button triggers a different HTTP method via router.">
					...
					<p class="text-muted-foreground mt-2 text-xs">Open DevTools Network tab to see the HTTP method used for each request.</p>
				</FeatureCard>

				<FeatureCard title="With Options" description="Manual visits support the same options as Link: preserveState, preserveScroll, replace, and more.">
					<div class="flex flex-wrap gap-3">
						<Button @click="visitWithOptions">GET (preserve state + scroll, replace)</Button>
					</div>
					<pre class="mt-3 overflow-auto rounded-md bg-muted p-3 text-xs leading-relaxed"><code>router.get(url, {}, {
  preserveState: true,
  preserveScroll: true,
  replace: true,
})</code></pre>
				...
				<FeatureCard title="Usage" description="Import router from @inertiajs/vue3 and call visit methods.">
					<pre class="overflow-auto rounded-md bg-muted p-3 text-xs leading-relaxed"><code>import { router } from '@inertiajs/vue3'
...
router.delete('/url')</code></pre>
				...
				<FeatureCard title="Generic Visit" description="The router.visit() method accepts a method option for full control.">
					<pre class="overflow-auto rounded-md bg-muted p-3 text-xs leading-relaxed"><code>router.visit('/url', {
  method: 'post',
  data: { name: 'John' },
  preserveState: true,
  onSuccess: (page) => { ... },
