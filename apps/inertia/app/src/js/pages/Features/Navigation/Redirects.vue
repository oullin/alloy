	...
	<AppLayout title="Redirects" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			...
			<div class="grid gap-6 lg:grid-cols-3">
				<FeatureCard title="Redirect Back" description="The server redirects back to this page with a flash message.">
					<div class="flex flex-wrap gap-3">
						<Button @click="redirectBack">Redirect back</Button>
					</div>
					<p class="text-muted-foreground mt-2 text-xs">Simulates a "redirect back" pattern commonly used after form submissions.</p>
				</FeatureCard>

				<FeatureCard title="Named Route" description="The server redirects to a specific named route with a flash message.">
					<div class="flex flex-wrap gap-3">
						<Button variant="outline" @click="redirectToRoute">Redirect to route</Button>
					</div>
					<p class="text-muted-foreground mt-2 text-xs">The server resolves a named route and sends a redirect response.</p>
				</FeatureCard>

				<FeatureCard title="External Redirect" description="The server responds with an external location, causing a full page load.">
					<div class="flex flex-wrap gap-3">
						<Button variant="outline" @click="externalRedirect">External redirect</Button>
					</div>
					<p class="text-muted-foreground mt-2 text-xs">
						This triggers
						<code class="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">window.location</code> to navigate to an external URL (inertiajs.com).
					</p>
				</FeatureCard>
			</div>
		</div>
