	...
	<AppLayout title="Optimistic Updates" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Contacts" description="Click the star button to toggle favorites. The UI updates optimistically.">
				...
				<div class="grid gap-6">
					<FeatureCard title="How It Works" description="Understanding optimistic updates in Inertia.">
						<div class="grid gap-2 text-sm">
							...
							<div class="rounded-md border p-3">
								...
								<p class="text-muted-foreground">When the response arrives, Inertia reconciles the server state with the UI. If the request failed, the UI reverts automatically.</p>
							</div>
						</div>
					</FeatureCard>
					...
					<FeatureCard title="Favorites" description="Current favorite contacts.">
						<div class="text-sm">
							<div v-if="favorites.length" class="grid gap-1">
								<div v-for="fav in favorites" :key="fav.id" class="flex items-center gap-2 rounded bg-yellow-50 px-3 py-2">
