...
<template>
	<AppLayout title="When Visible" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="When Visible" description="Load deferred data only when the component scrolls into the viewport. Scroll down to trigger each section." />

			<FeatureCard title="Introduction" description="The sections below will load their data lazily as you scroll down.">
				<p class="text-muted-foreground text-sm">
					WhenVisible uses an IntersectionObserver to detect when an element enters the viewport, then triggers a partial reload to fetch the associated data. This is ideal for long pages
			...
			<WhenVisible data="section3">
				...
				<FeatureCard title="Section 3" description="The final lazy-loaded section.">
					<div class="space-y-2">
						...
						<p class="text-muted-foreground text-sm">
							{{ section3?.content ?? 'Each section makes its own partial reload request to the server.' }}
						</p>
					</div>
				</FeatureCard>
			</WhenVisible>
