...
const breadcrumbs = [{ title: 'Features' }, { title: 'Data Loading' }, { title: 'Partial Reloads' }];
...
<template>
	<AppLayout title="Partial Reloads" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Partial Reloads" description="Reload only specific props without re-fetching the entire page data. This reduces server load and speeds up updates." />
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Users" description="Reloaded independently from other props.">
					<div class="space-y-2">
						<div v-for="user in users" :key="user.id ?? user.email" class="flex items-center gap-3 rounded-md border p-3">
							<div class="bg-primary/10 text-primary flex size-8 items-center justify-center rounded-full text-xs font-medium">
								{{ (user.name ?? 'U')[0] }}
							</div>
				...
				<div class="space-y-6">
					<FeatureCard title="Stats" description="Separate prop group that can be refreshed on its own.">
						<div class="grid grid-cols-2 gap-3">
							<div v-for="(value, key) in stats" :key="key" class="rounded-md border p-3 text-center">
