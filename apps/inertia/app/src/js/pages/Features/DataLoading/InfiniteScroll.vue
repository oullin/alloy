...
const breadcrumbs = [{ title: 'Features' }, { title: 'Data Loading' }, { title: 'Infinite Scroll' }];
...
<template>
	<AppLayout title="Infinite Scroll" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Infinite Scroll" description="Load more data incrementally using cursor-based pagination. Click the button below to fetch the next batch of contacts." />

			<FeatureCard title="Contacts" :description="`Showing ${allContacts.length} contacts`">
				<div class="space-y-2">
					<div v-for="contact in allContacts" :key="contact.id ?? contact.email" class="flex items-center gap-3 rounded-md border p-3">
						<div class="bg-primary/10 text-primary flex size-9 shrink-0 items-center justify-center rounded-full text-sm font-medium">
					...
					<p v-if="allContacts.length === 0" class="text-muted-foreground py-8 text-center text-sm">No contacts found.</p>
				</div>
				...
				<p v-else-if="allContacts.length > 0" class="text-muted-foreground mt-4 text-center text-sm">All contacts loaded.</p>
			</FeatureCard>
		</div>
	</AppLayout>
</template>
