...
function triggerFlash(kind: string) {
	...
	<AppLayout title="Flash Data" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			...
			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Trigger Flash Messages" description="Click a button to send a POST and receive a flash message.">
					...
					<p class="text-muted-foreground mt-4 text-sm">
						After clicking, the server sets a flash message and redirects back. The flash message appears in the header bar above and in the panel to the right.
					</p>
				</FeatureCard>
				...
				<FeatureCard title="Current Flash" description="What is currently in page.props.flash.">
					<div v-if="page.props.flash?.message" class="space-y-3">
						<div class="flex items-center gap-2">
							<Badge :variant="{ error: 'destructive', success: 'success', warning: 'warning' }[page.props.flash.kind] ?? 'default'">
						...
						<pre class="bg-muted overflow-auto rounded p-3 text-xs">{{ JSON.stringify(page.props.flash, null, 2) }}</pre>
					</div>
					<p v-else class="text-muted-foreground text-sm">No flash data. Click one of the buttons to trigger a flash message.</p>
				</FeatureCard>
			</div>
			...
			<FeatureCard title="How It Works">
				<div class="space-y-2 text-sm">
					<p>
						Flash data is set on the server during a request (typically before a redirect). Inertia includes it in
						<code class="bg-muted rounded px-1.5 py-0.5 text-xs">page.props.flash</code>
						as a shared prop.
					</p>
					<p>Flash data is automatically cleared after the next Inertia request, making it ideal for success/error notifications after form submissions.</p>
				</div>
			</FeatureCard>
		</div>
	</AppLayout>
