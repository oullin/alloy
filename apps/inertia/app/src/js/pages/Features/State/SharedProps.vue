...
<template>
	<AppLayout title="Shared Props" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Shared Props" description="Shared props are data that is available on every page without explicitly passing it. Access them via usePage().props." />

			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="All Shared Props" description="Every prop available via usePage().props on this page.">
					<div class="space-y-2">
						<div v-for="entry in propEntries" :key="entry.key" class="rounded-md border p-3">
							...
							<pre class="bg-muted mt-2 overflow-auto rounded p-2 text-xs">{{ JSON.stringify(entry.value, null, 2) }}</pre>
						</div>
					</div>
				</FeatureCard>
				...
				<div class="space-y-6">
						...
						<template v-if="page.props.auth?.user">
							<div class="space-y-2">
								<div class="flex items-center gap-3">
									<div class="bg-primary/10 text-primary flex size-10 items-center justify-center rounded-full font-medium">
					...
					<FeatureCard title="Routes" description="Shared route definitions.">
						<div v-if="page.props.routes" class="max-h-64 space-y-1 overflow-auto">
							<div v-for="(url, name) in page.props.routes" :key="name" class="flex items-center justify-between gap-2 rounded border px-2 py-1.5 text-xs">
					...
					<FeatureCard title="How It Works">
						<p class="text-sm">
							Shared props are defined on the server and automatically merged into every Inertia response. Common uses include the authenticated user, flash messages, app configuration, and
							route definitions. Access them anywhere with
