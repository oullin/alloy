...
<template>
	<AppLayout title="Manual Prefetch" :breadcrumbs="breadcrumbs">
		<div class="flex flex-1 flex-col gap-6 p-4 lg:p-6">
			<FeatureHeader title="Manual Prefetch" description="Programmatically prefetch a page using router.prefetch() for full control over when and what to prefetch." />

			<div class="grid gap-6 lg:grid-cols-2">
				<FeatureCard title="Trigger Prefetch" description="Click the button to manually prefetch a page.">
					<div class="space-y-4">
						<div class="flex items-center gap-3">
							<Button @click="triggerPrefetch" :disabled="prefetchStatus === 'prefetching'">
								{{ prefetchStatus === 'prefetching' ? 'Prefetching...' : 'Prefetch Link Prefetch Page' }}
							</Button>
						...
						<Button v-if="prefetchStatus === 'cached'" variant="outline" @click="navigateToPrefetched"> Navigate (instant!) </Button>

						<p class="text-muted-foreground text-sm">After prefetching, the navigation to that page will be instant because the data is already in the cache.</p>
					</div>
				</FeatureCard>
