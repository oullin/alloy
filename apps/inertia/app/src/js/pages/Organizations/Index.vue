...
const breadcrumbs = [{ title: 'CRM' }, { title: 'Organizations', href: organizationRoutes.index().url }];
	...
	<AppLayout title="Organizations" :breadcrumbs="breadcrumbs">
		<div class="flex h-full flex-1 flex-col gap-4 p-4">
			...
			<div class="space-y-2">
				...
				<div v-if="orgList.length === 0" class="text-muted-foreground py-12 text-center">No organizations found.</div>
			</div>
			...
			<div v-if="organizations.last_page > 1" class="flex items-center justify-center gap-1 py-4">
				<template v-for="(page, index) in pages" :key="index">
					...
					<Button v-else variant="outline" size="sm" :class="{ 'bg-accent font-bold': page === organizations.current_page }" @click="goToPage(page)">
						{{ page }}
					</Button>
				</template>
			</div>
