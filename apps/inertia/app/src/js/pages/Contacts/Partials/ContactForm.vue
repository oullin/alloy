		...
		<div class="flex flex-1 flex-col gap-6 p-4">
			<Card class="max-w-3xl">
				<CardHeader>
					...
					<CardDescription>Keep the CRM data aligned with the upstream demo structure.</CardDescription>
				</CardHeader>
				<CardContent>
					<form class="grid gap-6 md:grid-cols-2" @submit.prevent="submit">
						...
						<div class="md:col-span-2 flex items-center gap-3">
							...
							<span v-if="isEdit && form.isDirty" class="text-muted-foreground text-sm"> Unsaved changes </span>
							<span v-if="isEdit && form.recentlySuccessful" class="text-sm text-green-600"> Saved! </span>
						</div>
					</form>
				</CardContent>
			</Card>
