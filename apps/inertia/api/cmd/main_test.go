...
func TestResolveDistPath(t *testing.T) {
	t.Run("finds first candidate", func(t *testing.T) {
		...
		os.MkdirAll(filepath.Join(tmp, "storage", "apps", "inertia", "dist", "app"), 0o755)
		...
		if got != filepath.Clean("storage/apps/inertia/dist/app") {
			t.Errorf("got %q, want %q", got, "storage/apps/inertia/dist/app")
		}
	})
	...
	t.Run("finds repo storage candidate", func(t *testing.T) {
		...
		if err := os.MkdirAll(filepath.Join(tmp, "apps", "inertia", "api"), 0o755); err != nil {
			t.Fatalf("mkdir app cwd: %v", err)
		}

		if err := os.MkdirAll(filepath.Join(tmp, "storage", "apps", "inertia", "dist", "app"), 0o755); err != nil {
			t.Fatalf("mkdir dist dir: %v", err)
		}

		if err := os.Chdir(filepath.Join(tmp, "apps", "inertia", "api")); err != nil {
	...
	t.Run("uses env override", func(t *testing.T) {
		t.Setenv("ALLOY_INERTIA_DIST_PATH", "/tmp/alloy-inertia-dist")
		...
		if got != filepath.Clean("/tmp/alloy-inertia-dist") {
			t.Errorf("got %q, want %q", got, filepath.Clean("/tmp/alloy-inertia-dist"))
		}
	})
...
func TestResolveDatabasePath(t *testing.T) {
	...
	t.Run("finds repo storage from root", func(t *testing.T) {
		...
		if err := os.MkdirAll(filepath.Join(tmp, "storage", "apps"), 0o755); err != nil {
		...
		want := filepath.Clean("storage/apps/inertia/beacon.db")
