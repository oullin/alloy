...
describe('Tempo TypeScript behavior', () => {
	it.each(dateCases)('$name', (item) => {
		...
		if (item.expectedAddDaysIso !== undefined) {
			expect(parsed.addDays(item.addDays ?? 0).toISOString()).toBe(item.expectedAddDaysIso);
		}
	});
	...
	it('accepts both second and millisecond numeric timestamps', () => {
		expect(TempoImmutable.fromTimestamp(1_704_067_200).toISOString()).toBe('2024-01-01T00:00:00.000Z');

		expect(TempoImmutable.parse(1_704_067_200_000).toISOString()).toBe('2024-01-01T00:00:00.000Z');
	});
	...
	it('supports setters, unit arithmetic, boundaries, and overflow modes', () => {
		...
		expect(base.set({ day: 15, hour: 1, minute: 2, second: 3, millisecond: 4 }).toISOString()).toBe('2024-01-15T01:02:03.004Z');
		expect(base.addMonths(1).toDateString()).toBe('2024-03-02');
		expect(base.addMonthsNoOverflow(1).toDateString()).toBe('2024-02-29');
		expect(Tempo.parse('2024-02-29T00:00:00Z').addYearsNoOverflow(1).toDateString()).toBe('2025-02-28');
		expect(base.startOf('day').toISOString()).toBe('2024-01-31T00:00:00.000Z');
		expect(base.startOfWeek().toDateString()).toBe('2024-01-29');
		expect(base.endOfWeek({ weekStartsOn: 0 }).toDateString()).toBe('2024-02-03');
		expect(base.endOf('month').toISOString()).toBe('2024-01-31T23:59:59.999Z');
		expect(base.startOf('quarter').toISOString()).toBe('2024-01-01T00:00:00.000Z');
		expect(base.startOfQuarter().toISOString()).toBe('2024-01-01T00:00:00.000Z');
	...
	it('compares, diffs, rounds, and formats real instants', () => {
		...
		expect(base.diffAsDuration(earlier, { absolute: true }).toISOString()).toBe('P1DT2H34M45.600S');
		expect(base.diffAsDateInterval(earlier).toISOString()).toBe('P1DT2H34M45.600S');
		...
		expect(base.floorUnit('hour').toISOString()).toBe('2024-05-15T10:00:00.000Z');
		expect(base.ceilUnit('hour').toISOString()).toBe('2024-05-15T11:00:00.000Z');
		expect(base.roundUnit('hour').toISOString()).toBe('2024-05-15T11:00:00.000Z');
	...
	it('iterates periods and computes intervals', () => {
		const interval = new TempoInterval('2024-01-01T00:00:00Z', '2024-01-03T12:00:00Z');
		...
		const inverted = new TempoInterval('2024-01-03T12:00:00Z', '2024-01-01T00:00:00Z');

		expect(inverted.isInverted).toBe(true);
		expect(inverted.hours).toBe(-60);
		expect(inverted.invert().hours).toBe(60);
		expect(inverted.absolute().start.toISOString()).toBe('2024-01-01T00:00:00.000Z');
		expect(inverted.absolute().end.toISOString()).toBe('2024-01-03T12:00:00.000Z');

		const calendarInterval = new TempoInterval('2023-01-01T00:00:00Z', '2024-03-01T00:00:00Z');
		...
		expect(period.toArray().map((item) => item.toDateString())).toEqual(['2024-01-01', '2024-01-03', '2024-01-05']);
		...
		expect(period.filter((item) => item.day !== 3).map((item) => item.toDateString())).toEqual(['2024-01-01', '2024-01-05']);
		expect(period.map((item, index) => `${index}:${item.day}`)).toEqual(['0:1', '1:3', '2:5']);
		...
		expect(openPeriod.toArray().map((item) => item.toDateString())).toEqual(['2024-01-01', '2024-01-03']);
		...
		expect(reversePeriod.toArray().map((item) => item.toDateString())).toEqual(['2024-01-05', '2024-01-03', '2024-01-01']);
		expect(() =>
	...
	it('parses known token formats and applies offsets', () => {
		...
		expect(Tempo.fromFormat('05-15-24 10:34 PM', 'MM-DD-YY hh:mm A').toISOString()).toBe('2024-05-15T22:34:00.000Z');

		expect(Tempo.fromFormat('Wednesday, May 15th 2024 10:34 PM', 'dddd, MMMM Do YYYY hh:mm A').toISOString()).toBe('2024-05-15T22:34:00.000Z');

		expect(Tempo.fromFormat('Wed, May 15 2024', 'ddd, MMM D YYYY').toDateString()).toBe('2024-05-15');
	...
	it('supports ISO week metadata and weekday navigation', () => {
		...
		expect(monday.next('friday').toDateTimeString()).toBe('2024-01-05 12:30:00');
		expect(monday.previous('friday').toDateTimeString()).toBe('2023-12-29 12:30:00');
		expect(monday.nextOrSame('monday').toDateTimeString()).toBe('2024-01-01 12:30:00');
		expect(monday.previousOrSame('monday').toDateTimeString()).toBe('2024-01-01 12:30:00');
		expect(monday.nextOrSame('friday').toDateTimeString()).toBe('2024-01-05 12:30:00');
		expect(monday.previousOrSame('friday').toDateTimeString()).toBe('2023-12-29 12:30:00');
	...
	it('parses, normalizes, serializes, and applies durations', () => {
		...
		expect(Tempo.parse('2024-01-31T00:00:00Z').addDuration('P1M').toDateString()).toBe('2024-03-02');
		expect(
	...
	it('adds business days and compares same calendar units', () => {
		...
		expect(friday.addWeekdays(1).toDateTimeString()).toBe('2024-05-20 10:00:00');
		expect(friday.addWeekdays(3).toDateTimeString()).toBe('2024-05-22 10:00:00');
		expect(wednesday.subWeekdays(3).toDateTimeString()).toBe('2024-05-17 10:00:00');
		...
		expect(wednesday.diffInDaysFiltered((item) => item.isMonday(), friday)).toBe(1);
		expect(wednesday.diffInHoursFiltered((item) => item.hour === 12, friday)).toBe(5);
		...
		expect(friday.isSameAs('YYYY-MM-DD HH:mm', '2024-05-17T23:59:59Z')).toBe(false);
	...
	it('clamps ranges, averages instants, and detects unit boundaries', () => {
		...
		expect(Tempo.parse('2024-05-01T00:00:00Z').clamp('2024-05-10T00:00:00Z', '2024-05-20T00:00:00Z').toISOString()).toBe('2024-05-10T00:00:00.000Z');
		expect(base.clamp('2024-05-10T00:00:00Z', '2024-05-20T00:00:00Z').toISOString()).toBe('2024-05-15T12:00:00.000Z');
		expect(Tempo.parse('2024-05-30T00:00:00Z').clamp('2024-05-10T00:00:00Z', '2024-05-20T00:00:00Z').toISOString()).toBe('2024-05-20T00:00:00.000Z');

		expect(base.average('2024-05-17T12:00:00Z').toISOString()).toBe('2024-05-16T12:00:00.000Z');
		expect(Tempo.average('2024-05-15T00:00:00Z', '2024-05-17T00:00:00Z').toISOString()).toBe('2024-05-16T00:00:00.000Z');
		...
		expect(base.startOfMillisecond().toISOString()).toBe('2024-05-15T12:00:00.000Z');
		expect(base.endOfMillisecond().toISOString()).toBe('2024-05-15T12:00:00.000Z');
		expect(base.isStartOfMillisecond()).toBe(true);
		expect(base.isEndOfMillisecond()).toBe(true);
		expect(Tempo.parse('2024-05-15T12:34:56.789Z').startOfSecond().toISOString()).toBe('2024-05-15T12:34:56.000Z');
		expect(Tempo.parse('2024-05-15T12:34:56.789Z').endOfSecond().toISOString()).toBe('2024-05-15T12:34:56.999Z');
		...
		expect(Tempo.parse('2020-01-01T00:00:00Z').isStartOfUnit('decade')).toBe(true);
		expect(Tempo.parse('2029-12-31T23:59:59.999Z').isEndOfDecade()).toBe(true);
		expect(Tempo.parse('2029-12-31T23:59:59.999Z').isEndOfUnit('decade')).toBe(true);
		expect(base.startOfCentury().toDateTimeString()).toBe('2001-01-01 00:00:00');
		...
		expect(base.startOfMillennium().toDateTimeString()).toBe('2001-01-01 00:00:00');
		expect(base.endOfMillennium().toDateTimeString()).toBe('3000-12-31 23:59:59');
		expect(Tempo.parse('2001-01-01T00:00:00Z').isStartOfMillennium()).toBe(true);
		expect(Tempo.parse('3000-12-31T23:59:59.999Z').isEndOfMillennium()).toBe(true);
	...
	it('serializes named date formats and maps components', () => {
		...
		expect(tempo.toIso8601ZuluString('millisecond')).toBe('2024-05-15T12:34:56.789Z');
		expect(tempo.toRfc3339String('millisecond')).toBe('2024-05-15T21:34:56.789+09:00');
	...
	it('sets explicit components including zero values', () => {
		...
		expect(tempo.setTime(0, 0, 0, 0).toISOString()).toBe('2024-05-15T00:00:00.000Z');
		...
		expect(tempo.setDateTime(2025, 1, 2, 3, 4, 5, 6).toISOString()).toBe('2025-01-02T03:04:05.006Z');
		expect(tempo.setDateFrom('2025-01-02T03:04:05.006Z').toDateTimeString()).toBe('2025-01-02 12:34:56');
		expect(tempo.setTimeFrom('2025-01-02T03:04:05.006Z').toDateTimeString()).toBe('2024-05-15 03:04:05');
		...
		expect(tempo.avoidMutation().toISOString()).toBe('2024-05-15T12:34:56.789Z');
	...
	it('scopes translator behavior through composable runtimes', () => {
		...
		expect(replaced.translate('greeting', { name: 'Tempo' })).toBe('Salut Tempo');
		expect(first.translate('greeting', { name: 'Tempo' })).toBe('Hello Tempo');
	});
